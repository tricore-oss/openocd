...
static inline void aurix_eflash_reset_to_read(struct flash_bank *bank) {
  ...
  if (ret) {
    LOG_ERROR(
        "Failed to execute reset to read. Please reset device to continue");
  }
}

static inline int aurix_eflash_handle_error(struct flash_bank *bank,
                                            uint32_t sts_bit,
                                            int64_t timeout_ms) {
  struct aurix_eflash_bank *tc3x_bank = bank->driver_priv;
  struct ocmts *ocmts = target_to_tricore(bank->target)->ocmts;
  ...
  while (start_time + timeout_ms > timeval_ms()) {
    ret = ocmts_queue_read_u32(
        ocmts, tc3x_bank->reg_addr + tc3x_bank->err_offset, &flash_err);
    if (ret) {
      goto status_err;
    }
    ret = ocmts_queue_read_u32(
        ocmts, tc3x_bank->reg_addr + tc3x_bank->sts_offset, &flash_busy);
    if (ret) {
      goto status_err;
    }
    ret = ocmts_run(ocmts);
...
static inline int aurix_eflash_check_busy(struct flash_bank *bank) {
  ...
  struct ocmts *ocmts = target_to_tricore(bank->target)->ocmts;
  uint32_t flash_sts;
  int ret = ocmts_io_read_u32(
      ocmts, aurix_bank->reg_addr + aurix_bank->sts_offset, &flash_sts);
  if (ret) {
...
static inline int aurix_eflash_enter_page_mode(struct flash_bank *bank) {
  ...
  ret = ocmts_io_read_u32(ocmts, aurix_bank->reg_addr + aurix_bank->sts_offset,
                          &flash_sts);
  if (ret) {
    LOG_ERROR("Failed to read flash status");
    return ERROR_FLASH_OPERATION_FAILED;
...
int aurix_eflash_erase(struct flash_bank *bank, unsigned int first,
                       unsigned int last) {
  struct aurix_eflash_bank *aurix_bank = bank->driver_priv;
  struct ocmts *ocmts = target_to_tricore(bank->target)->ocmts;
  int ret;
  ...
  while (first <= last) {
    ...
    uint32_t sectors_to_boundary = MIN(
        last - first + 1,
        aurix_bank->phys_sector_size / aurix_bank->sector_size -
            (first % (aurix_bank->phys_sector_size / aurix_bank->sector_size)));
    /* Limit to logical sector erase count. */
    uint32_t sector_count = MIN(aurix_bank->num_erase_sectors,
                                MIN(last - first + 1, sectors_to_boundary));
    ...
    ret = ocmts_queue_write_u32(ocmts, aurix_bank->cmd_addr + 0xAA58,
                                &sector_count);
    if (ret) {
      goto sequence_err;
    }
    ret = ocmts_queue_write_u32(ocmts, aurix_bank->cmd_addr + 0xAAA8,
                                &(uint32_t){0x80});
    if (ret) {
      goto sequence_err;
    }
  ...
  sequence_err:
    ...
    ret = aurix_eflash_handle_error(bank, aurix_bank->busy_bit,
                                    aurix_bank->erase_timeout_ms);
    if (ret) {
      return ret;
    }
  }
...
static int aurix_eflash_write(struct flash_bank *bank, const uint8_t *buffer,
  ...
  if (offset & ~(aurix_bank->page_size - 1) ||
      count % aurix_bank->page_size != 0) {
  ...
  while (page_offset < count) {
    ...
    const uint32_t copy_size =
        burst_mode ? aurix_bank->burst_size : aurix_bank->page_size;
    uint32_t i;
    ...
    if (aurix_df_check_if_tc4x(bank->target->tap->idcode)) {
      for (i = 0; i < copy_size && page_offset + i < count; i += 4) {
        ret = ocmts_queue_write_u32(ocmts, aurix_bank->cmd_addr + 0x55F4,
      ...
      for (i = 0; i < copy_size && page_offset + i < count; i += 4) {
        ret = ocmts_queue_write_u32(
            ocmts, aurix_bank->cmd_addr + 0x55F0 + ((i % 8) == 0 ? 0 : 4),
            (void *)(buffer + page_offset + i));
        if (ret) {
          goto err;
        }
  ...
  err:
    if (ret) {
      LOG_ERROR("Failed to execute flash program sequence at address: 0x%08x, "
                "data size: %u",
                addr, copy_size);
      aurix_eflash_reset_to_read(bank);
      return ERROR_FLASH_OPERATION_FAILED;
    ...
    if (aurix_eflash_handle_error(bank, aurix_bank->busy_bit,
                                  aurix_bank->write_timeout_ms) != ERROR_OK) {
      return ERROR_FLASH_OPERATION_FAILED;
    }
  }
...
static int aurix_eflash_read(struct flash_bank *bank, uint8_t *buffer,
                             uint32_t offset, uint32_t count) {
...
FLASH_BANK_COMMAND_HANDLER(tc3x_flash_bank_command) {
  ...
  if (is_pflash) {
    if (bank->size != 1 * 1024 * 1024 && bank->size != 2 * 1024 * 1024 &&
        bank->size != 3 * 1024 * 1024) {
      LOG_ERROR("Invalid pflash bank size. Size should be 1MB, 2MB or 3MB.");
      return ERROR_FLASH_BANK_INVALID;
    }
    if (bank->base % (1 * 1024 * 1024) != 0) {
      LOG_ERROR(
          "Invalid pflash bank base address. Base should be aligned to 1MB.");
      return ERROR_FLASH_BANK_INVALID;
    }
  } else if (is_ucb) {
  ...
  tc3x_bank->num_erase_sectors = is_ucb ? 1
                                 : is_dflash
                                     ? 256 * 1024 / tc3x_bank->sector_size
...
FLASH_BANK_COMMAND_HANDLER(tc4x_flash_bank_command) {
  ...
  if (is_pflash0) {
    ...
    if (bank->base % (2 * 1024 * 1024) != 0) {
      LOG_ERROR(
          "Invalid pflash0 bank base address. Base should be aligned to 2MB.");
      return ERROR_FLASH_BANK_INVALID;
    }
  } else if (is_pflash1) {
    if (bank->size != 1 * 1024 * 1024) {
    ...
    if (bank->size != 1024 * 1024 && bank->size != 512 * 1024 &&
        bank->size != 128 * 1024) {
      LOG_ERROR(
          "Invalid dflash bank size. Size should be 128KB, 512KB or 1MB.");
      return ERROR_FLASH_BANK_INVALID;
    }
  } else if (is_dflash1) {
    if (bank->size != 128 * 1024) {
  ...
  tc4x_bank->busy_bit = is_pflash0   ? bank->bank_number
                        : is_pflash1 ? 18
                        : is_dflash0 ? 16
                                     : 17;
  ...
  tc4x_bank->num_erase_sectors = is_ucb ? 1
                                 : is_dflash
                                     ? 256 * 1024 / tc4x_bank->sector_size
