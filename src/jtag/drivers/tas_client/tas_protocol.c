...
int tas_client_connect(struct tas_client *client, int sock) {
  ...
  err = recv(sock, (char *)&rsp_server_connect,
             sizeof(tas_pl1rsp_server_connect_st), 0);
  if (err < 0) {
    return ERROR_FAIL;
  }
...
int tas_client_session_start(struct tas_client *client, const char *device) {
  ...
  if (recv(client->sock, (char *)&rsp_session_start,
  ...
  client->max_pl2rq_pkt_size =
      MIN(max_pl2rq_pkt_size, rsp_session_start.con_info.max_pl2rq_pkt_size);
  client->max_pl2rsp_pkt_size =
      MIN(max_pl2rsp_pkt_size, rsp_session_start.con_info.max_pl2rsp_pkt_size);
  client->pl0_max_num_rw =
      MIN(pl0_max_num_rw, rsp_session_start.con_info.pl0_max_num_rw);

  return 0;
}

int tas_client_device_connect(struct tas_client *client,
                              tas_dev_con_feat_et dev_con_feat) {
  ...
  if (recv(client->sock, (char *)&rsp_device_connect,
...
int tas_client_get_targets(struct tas_client *client,
                           tas_target_info_st **targets, size_t *target_num) {
  ...
  if (recv(client->sock, (char *)&rsp_get_targets,
           sizeof(tas_pl1rsp_get_targets_st), 0) < 0) {
    return ERROR_FAIL;
  }
  ...
  *target_num = rsp_get_targets.num_target;
  ...
  if (*target_num > 0) {
    *targets = calloc(*target_num, sizeof(tas_target_info_st));
    ...
    if (recv(client->sock, (char *)*targets,
             *target_num * sizeof(tas_target_info_st), 0) < 0) {
      return ERROR_FAIL;
    }
  }
...
int tas_client_send_pl0(struct tas_client *client, uint8_t con_id,
                        uint8_t *tx_buffer, uint32_t tx_len, uint8_t *rx_buffer,
                        size_t *rx_len, tas_pl0rsp_st **pl0_resp,
                        size_t pl0_elements) {
  uint32_t packet_size;
...
int tas_client_execute_mem_req(struct tas_client *client, uint8_t addr_map,
                               struct tas_client_mem_req *mem_req) {
  int err;
  size_t tx_offset = 0, rx_offset = 0;
  size_t rx_size =
      sizeof(tas_pl1rsp_pl0_start_st) + sizeof(tas_pl1rsp_pl0_end_st) + 4;
  char tx_buf[client->max_pl2rq_pkt_size];
  char rx_buf[client->max_pl2rsp_pkt_size];
  uint32_t packet_size;

  if (mem_req->length != 1 && mem_req->length != 2 && mem_req->length != 4 &&
      mem_req->length != 8 && mem_req->length % 4 != 0 &&
      mem_req->length > 1024) {
    return ERROR_FAIL;
  }
  ...
  tas_pl0rq_addr_map_st rq_addr_map = {
      .addr_map = addr_map, .cmd = TAS_PL0_CMD_ADDR_MAP, .wl = 0};
  ...
  if (packet_size > client->max_pl2rsp_pkt_size) {
    LOG_DEBUG(
        "Response packet size %u exceeds maximum PL2 response packet size %u",
        packet_size, client->max_pl2rsp_pkt_size);
    return ERROR_FAIL;
  }
  ...
  if (packet_size > rx_size) {
    LOG_DEBUG("Response packet size %u exceeds expected size %zu", packet_size,
              rx_size);
    return ERROR_FAIL;
  }
...
int tas_client_execute_mem_reqs(struct tas_client *client, uint8_t addr_map,
                                struct tas_client_mem_req *mem_reqs,
                                size_t mem_req_num) {
  int err;
  ...
  while (cur_mem_req < mem_req_num) {
    size_t tx_offset = 4;
    size_t rx_size =
        4 + sizeof(tas_pl1rsp_pl0_start_st) + sizeof(tas_pl1rsp_pl0_end_st);
    tas_pl1rq_pl0_start_st rq_start = {
        .cmd = TAS_PL1_CMD_PL0_START,
        .wl = 1,
        .con_id = 0,
    ...
    size_t i;
    for (i = cur_mem_req;
         i < mem_req_num && rq_end.num_pl0_rw < client->pl0_max_num_rw;
         i++, rq_end.num_pl0_rw++) {
      if (mem_reqs[i].length != 1 && mem_reqs[i].length != 2 &&
          mem_reqs[i].length != 4 && mem_reqs[i].length != 8 &&
          mem_reqs[i].length % 4 != 0 && mem_reqs[i].length > 1024) {
        return ERROR_FAIL;
      }
      ...
      if (tx_offset + sizeof(tas_pl0rq_base_addr32_st) +
              ...
              client->max_pl2rq_pkt_size ||
          rx_size + (mem_reqs[i].is_read ? sizeof(tas_pl0rsp_rd_st) + words * 4
      ...
      if (mem_reqs[i].is_read) {
        ...
        if (mem_reqs[i].length > 8) {
          memcpy(tx_buffer + tx_offset, &pl0rq_read,
