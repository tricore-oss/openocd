...
/*! \ingroup Protocol_Definition */
typedef enum {
	TAS_PKT_PROTOC_VER_1 = 1,	/*!< \brief Initial protocol version (value 0 is
					 * unknown/undefined/unused etc.) */
} tas_protoc_ver_et;

/*! \brief Collection of used port numbers. */
/*! \details Non-standard port numbers are used. Current list can be found at
...
enum {
	...
	TAS_PORT_NUM_DEVICE_DEFAULT = 24823,	/*!< \brief TasDeviceServer locally connected to
						 * TasServer */
};
...
/*! \ingroup Protocol_Definition */
enum {
	TAS_NAME_LEN64 = 64,			/*!< \brief Used for specifying a 64 Byte long
						 * string */
	TAS_NAME_LEN32 = 32,			/*!< \brief Used for specifying a 32 Byte long
						 * string */
	TAS_NAME_LEN16 = 16,			/*!< \brief Used for specifying a 16 Byte long
						 * string */
	TAS_NAME_LEN8  =  8,			/*!< \brief Used for specifying an 8 Byte long
						 * string */
...
typedef enum {
	...
	TAS_PL_ERR_NOT_SUPPORTED  = 0x12,	/*!< \brief Command or parameter not supported by
						 * this implementation or setup */
	TAS_PL_ERR_USAGE          = 0x14,	/*!< \brief Not correctly used (e.g. wrong sequence
						 * of commands) */
	...
	TAS_PL1_ERR_DEV_RESET     = 0x44,	/*!< \brief Device reset was detected. Sent to all
						 * clients as an unsolicited PL2 packet */
	...
	TAS_PL0_ERR_DEV_LOCKED    = TAS_PL1_ERR_DEV_LOCKED,	/*!< \brief no     Device is locked
								 * (no access possible) */
	TAS_PL0_ERR_DEV_ACCESS    = TAS_PL1_ERR_DEV_ACCESS,	/*!< \brief no     Device access
								 * (e.g. DAP connection) */
	TAS_PL0_ERR_ACC_MODE      = 0x89,			/*!< \brief no     Set acc_mode is
								 * not supported */
	TAS_PL0_ERR_ADDR_MAP      = 0x8A,			/*!< \brief no     Set addr_map is
								 * not supported */
	TAS_PL0_ERR_ADDR_BLOCKED  = 0x8B,			/*!< \brief no     TasAgent
								 * protection prevents access to
								 * this address */
	TAS_PL0_ERR_DATA          = 0x8D,			/*!< \brief tried  RW access was not
								 * (completely - blk) successful
								 * (e.g. device internal bus error) */
	TAS_PL0_ERR_CONSEQUENTIAL = 0x98,			/*!< \brief impl   Consequential
								 * error since a previous PL0
								 * command failed. Whether the
								 * access took place is
								 * implementation specific.
								 * Recommendation is no. */

	TAS_PL0_ERR_HIGHEST       = 0x9F,			/*!< \brief no     Delimiter for PL0
								 * error codes */
...
typedef enum {
	...
	/* PL0 RW commands */
	TAS_PL0_CMD_ACCESS_MODE     = 0x10,		/*!< \brief No response pkt. Sets the access
							 * mode. */
	TAS_PL0_CMD_ADDR_MAP        = 0x11,	/*!< \brief No response pkt. Sets the address map to
						 * be used. */
	TAS_PL0_CMD_BASE_ADDR32     = 0x15,	/*!< \brief No response pkt. Sets the a 32-bit base
						 * address for r/w commands that follow */
	TAS_PL0_CMD_BASE_ADDR64     = 0x16,	/*!< \brief No response pkt. Sets the a 64-bit base
						 * address for r/w commands that follow */
	TAS_PL0_CMD_WR8             = 0x20,	/*!< \brief TAS_PL0_ERR_. Issues a request for an
						 * 8-bit write operation. */
	TAS_PL0_CMD_RD8             = 0x21,	/*!< \brief TAS_PL0_ERR_. Issues a request for an
						 * 8-bit read operation. */
	TAS_PL0_CMD_WR16            = 0x22,	/*!< \brief TAS_PL0_ERR_. Issues a request for a
						 * 16-bit write operation. */
	TAS_PL0_CMD_RD16            = 0x23,	/*!< \brief TAS_PL0_ERR_. Issues a request for a
						 * 16-bit read operation. */
	TAS_PL0_CMD_WR32            = 0x24,	/*!< \brief TAS_PL0_ERR_. Issues a request for a
						 * 32-bit write operation. */
	TAS_PL0_CMD_RD32            = 0x25,	/*!< \brief TAS_PL0_ERR_. Issues a request for a
						 * 32-bit read operation. */
	TAS_PL0_CMD_WR64            = 0x26,	/*!< \brief TAS_PL0_ERR_. Issues a request for a
						 * 64-bit write operation. */
	TAS_PL0_CMD_RD64            = 0x27,	/*!< \brief TAS_PL0_ERR_. Issues a request for a
						 * 64-bit read operation. */
	TAS_PL0_CMD_WRBLK           = 0x2A,	/*!< \brief TAS_PL0_ERR_. Issues a request for a
						 * block write operation. */
	TAS_PL0_CMD_FILL            = 0x2B,	/*!< \brief TAS_PL0_ERR_. Issues a request for a
						 * fill operation. */
	TAS_PL0_CMD_RDBLK           = 0x2C,	/*!< \brief TAS_PL0_ERR_. Issues a request for a
						 * block read operation. */
	TAS_PL0_CMD_RDBLK1KB        = 0x2D,	/*!< \brief TAS_PL0_ERR_NO_ERROR (used for response
						 * if 1kb data read without error). */
	/*! Issues a request or 1KB block read operation. */

	/* PL1 commands */
	TAS_PL1_CMD_SERVER_CONNECT  = 0x80,		/*!< \brief Used for establishing a
							 * connection with a server. */

	TAS_PL1_CMD_GET_TARGETS     = 0x86,	/*!< \brief Used for requesting a list of targets
						 * connected to a server. */
	TAS_PL1_CMD_GET_CLIENTS     = 0x87,	/*!< \brief Used for requesting a list of clients
						 * connected to a server. */

	TAS_PL1_CMD_SESSION_START   = 0xA1,	/*!< \brief Used for opening a connection session. */
	TAS_PL1_CMD_PING            = 0xA2,	/*!< \brief Used for checking the connection which
						 * was established with session start. */
	TAS_PL1_CMD_DEVICE_CONNECT  = 0xA8,	/*!< \brief Used for connecting to a target (hot
						 * attach),or  with optional reset or reset and
						 * halt. */
	TAS_PL1_CMD_DEVICE_RESET_COUNT = 0xAA,	/*!< \brief Used for checking the number of times a
						 * target was reset. */

	TAS_PL1_CMD_GET_CHALLENGE   = 0xAC,		/*!< \brief TBD. */
	TAS_PL1_CMD_SET_DEVICE_KEY  = 0xAD,		/*!< \brief Set a device key for device
							 * unlocking. */

	TAS_PL1_CMD_PL0_START       = 0xB0,		/*!< \brief Used for indicating a start of
							 * PL0 sequence. */
	TAS_PL1_CMD_PL0_END         = 0xB1,		/*!< \brief Used for indicating an end of
							 * PL0 sequence. */

	TAS_PL1_CMD_CHL_SUBSCRIBE   = 0xC0,		/*!< \brief Used for subscribing to a
							 * channel number. */
	TAS_PL1_CMD_CHL_UNSUBSCRIBE = 0xC2,		/*!< \brief Used for unsubscribing from a
							 * subscribed channel number. */
	TAS_PL1_CMD_CHL_MSG_C2D     = 0xCC,		/*!< \brief Send a message from a client to
							 * a device. */
...
typedef enum {
	...
	TAS_DEV_CON_FEAT_UNLOCK_CNR     = 0x0030,	/*!< \brief Unlocking with challenge and
							 * response protocol */
	TAS_DEV_CON_FEAT_UNKNOWN        = 0x4000,	/*!< \brief Connection to an unknown device.
							 * In respect to JTAG ID. */
	TAS_DEV_CON_FEAT_UNKNOWN_RESET  = 0x4001,	/*!< \brief Triggers a reset of unknown
							 * device. In respect to JTAG ID. */
} tas_dev_con_feat_et;
...
/*! \ingroup Protocol_Definition */
typedef enum {
	TAS_DEV_ULCRO_UDID     = 0x01,	/*!< \brief Challenge is the unique device ID which can be
					 * read from the locked device */
	...
	TAS_DEV_ULCRO_CUSTOM0  = 0xC0,	/*!< \brief Option 0 for custom protocol for retrieving the
					 * challenge */
...
typedef enum {
	...
	TAS_DEV_ULO_AFTER_RST_BY_FW    = 0x08,	/*!< \brief Unlocking by FW before application SW is
						 * started */
	TAS_DEV_ULO_AFTER_RST_BY_APPSW = 0x09,	/*!< \brief Unlocking by application SW in a time
						 * window after reset */
...
/*! \ingroup Protocol_Definition */
typedef enum {
	TAS_PL0_ACC_MODE_DEFAULT = 0x0000,	/*!< \brief Default RW access mode for the target
						 * device */
} tas_pl0_acc_mode_et;
...
/*! \ingroup Packet_Level_1 */
typedef struct {
	char server_name[TAS_NAME_LEN64];	/*!< \brief storage for c-string containing the
						 * server name max 64 bytes long */
	uint16_t v_minor;		/*!< \brief Version minor */
	uint16_t v_major;		/*!< \brief Version major */
	uint32_t supp_protoc_ver;	/*!< \brief Supported protocol versions.  tas_protoc_ver_et
					 * defines the bit number */
	uint32_t supp_chl_target;	/*!< \brief Supported channel targets.    tas_chl_target_et
					 * defines the bit number */
	uint32_t supp_trc_type;		/*!< \brief Supported trace stream types. tas_trc_type_et
					 *   defines the bit number */
	uint32_t reserved[4];		/*!< \brief Reserved field: 0 */
	char date[16];			/*!< \brief String from __DATE__ macro at compile time */
	uint64_t start_time_us;		/*!< \brief Time the server was started, as microseconds
...
typedef struct {
	...
	uint8_t err;		/*!< \brief Error code: TAS_PL_ERR_NO_ERROR, TAS_PL1_ERR_CMD_FAILED
				 * or TAS_PL1_ERR_SERVER_LOCKED */
	uint32_t reserved1;	/*!< \brief Reserved field: Ensures no padding by compiler for 64
				 * bit alignment */
...
/*! \ingroup Packet_Level_1 */
typedef struct {
	char identifier[TAS_NAME_LEN64];	/*!< \brief Unique access HW name or IP address of
						 * device */
	uint32_t device_type;	/*!< \brief Device type identifier (IEEE 1149.1 device ID). 0 if no
				 * device connected */
...
typedef struct {
	...
	uint8_t start_index;	/*!< \brief Start index of targets' list. If 0 the list of all
				 * targets is sampled by the TasServer */
	uint8_t reserved;	/*!< \brief Reserved field: 0 */
...
/*! \ingroup Packet_Level_1 */
typedef struct {
	uint8_t wl;			/*!< \brief Response size (excluding header): 1...255 ->
					 * TAS_MAX_PKT_SIZE_1KB */
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL1_CMD_GET_TARGETS */
	uint8_t reserved;	/*!< \brief Reserved field: 0 */
	uint8_t err;		/*!< \brief Error response: TAS_PL_ERR_NO_ERROR or
				 * TAS_PL1_ERR_SERVER_LOCKED */
...
typedef struct {
	char client_name[TAS_NAME_LEN32];	/*!< \brief Client name as a c-string */
	char user_name[TAS_NAME_LEN16];			/*!< \brief Username of a user using this
							 * client as a c-string */
	uint32_t client_pid;		/*!< \brief Process ID of a client */
	uint8_t client_type;		/*!< \brief \ref tas_client_type_et */
	uint8_t reserved[3];		/*!< \brief Reserved field: 0 */
	uint64_t client_connect_time;	/*!< \brief Time in us since since the server start time
					 * \ref tas_server_info_st.start_time_us */
	uint64_t num_byte_c2s;			/*!< \brief Number of Bytes sent from a client to a
						 * server */
	uint64_t num_byte_s2c;			/*!< \brief Number of Bytes received by a client
						 * from a server */
...
typedef struct {
	...
	char identifier[TAS_NAME_LEN64];	/*!< \brief Target identifier. TAS_PL_ERR_PARAM if
						 * not valid */
...
/*! \ingroup Packet_Level_1 */
typedef struct {
	uint8_t wl;		/*!< \brief Response size (excluding header): 7...255 ->
				 * TAS_MAX_PKT_SIZE_1KB */
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL1_CMD_GET_CLIENTS */
	uint8_t reserved;	/*!< \brief Reserved field: 0 */
	uint8_t err;		/*!< \brief Error code: TAS_PL_ERR_NO_ERROR, TAS_PL_ERR_PARAM or
				 * TAS_PL1_ERR_SERVER_LOCKED */
	...
	char session_name[TAS_NAME_LEN16];	/*!< \brief Session name and time only if
						 * start_index is 0 */
	uint64_t session_start_time_us;			/*!< \brief Time in microseconds since
							 * tas_server_info_st.start_time_us */
...
typedef struct {
	...
	uint32_t device_type;		/*!< \brief Device type identifier (IEEE 1149.1 device ID).
					 * 0 if no device connected */
	uint32_t device_id[4];		/*!< \brief 128 bit unique device ID */
	...
	uint8_t dev_con_phys;		/*!< \brief \ref tas_dev_con_phys_et */
	uint8_t pl0_max_num_rw;		/*!< \brief Maximum number of RW transactions in a PL1
					 * packet */

	uint16_t pl0_rw_mode_mask;	/*!< \brief Mask for supported RW access mode bits */
	uint16_t pl0_addr_map_mask;	/*!< \brief Mask for supported address maps. Bit 15
					 * represents TAS_AM15. */

	uint16_t msg_length_c2d;	/*!< \brief Maximum length of messages sent from client to
					 * device. 0 if not ClientChl or channels not supported */
	uint16_t msg_length_d2c;	/*!< \brief Maximum length of messages sent from device to
					 * client. 0 if not ClientChl or channels not supported */

	uint8_t msg_num_c2d;		/*!< \brief Maximum number of messages sent from client to
					 * device */
	uint8_t msg_num_d2c;		/*!< \brief Maximum number of messages sent from device to
					 * client */
	uint16_t reserved;				/*!< \brief Reserved field: 0 */
...
/*! \details Includes an extension to \ref tas_con_info_d2s_st which are only transmitted between a
 * TasServer and a TasClient */
/*! \ingroup Packet_Level_1 */
typedef struct {
	uint32_t max_pl2rq_pkt_size;	/*!< \brief Maximum size of PL2 request packet */
	uint32_t max_pl2rsp_pkt_size;	/*!< \brief Maximum size if PL2 response packet */
	...
	uint8_t msg_num_c2d;		/*!< \brief Maximum number of messages sent from client to
					 * device that will be buffered in the device */
	uint8_t msg_num_d2c;		/*!< \brief Maximum number of messages sent from device to
					 * client that will be buffered in the device */
	uint16_t reserved;				/*!< \brief Reserved field: 0 */

	/* Extensions to tas_con_info_d2s_st which are only transmitted between TasServer and
	 * TasClient */

	char identifier[TAS_NAME_LEN64];		/*!< \brief Unique access HW name or IP
							 * address of device. Empty if not
							 * connected. */
	uint32_t device_id_hash;				/*!< \brief 32 bit hash of
								 * device_id, calculated by
								 * TasClient */
	char device_id_hash_str[TAS_NAME_LEN8];		/*!< \brief ASCII string generated with
							 * tasutil_hash32_to_str() */
...
/*! \details Valid for device - server connection. Includes the connection information \ref
 * tas_con_info_d2s_st. */
/*! \ingroup Packet_Level_1 */
typedef struct {
	...
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL1_CMD_PING */
	uint8_t con_id;		/*!< \brief Connection identifier of TasDispatcher */
	uint8_t err;		/*!< \brief TAS_PL_ERR_NO_ERROR (check acc_hw and device type in
				 * con_info) */
	...
	uint8_t num_instances;		/*!< \brief Number of matches for the identifier from \ref
					 * tas_pl1rq_device_connect_st */
	uint8_t reserved;		/*!< \brief Reserved field: 0 */

	tas_con_info_d2s_st con_info_d2s;	/*!< \brief Connection information on the device -
						 * server level */

} tas_pl1rsp_ping_d2s_st;

/*! \brief Response to \ref tas_pl1rq_ping_st or \ref tas_pl1rq_session_start_st. */
/*! \details Valid for client - server connection. Includes the full connection information \ref
 * tas_con_info_st. */
/*! \ingroup Packet_Level_1 */
typedef struct {
	uint8_t wl;		/*!< \brief Response size (excluding header): 31 */
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL1_CMD_SESSION_START or
				 * TAS_PL1_CMD_PING */
	uint8_t con_id;		/*!< \brief Connection identifier of TasDispatcher */
	uint8_t err;		/*!< \brief TAS_PL_ERR_NO_ERROR (check acc_hw and device type in
				 * con_info) */
...
typedef struct {
	...
	uint16_t ulcro;		/*!< \brief From where to get a challenge: \ref
				 * tas_dev_unlock_cr_option_et */
...
/*! \ingroup Packet_Level_1 */
typedef struct {
	uint8_t wl;		/*!< \brief Response size (excluding header): 1 ... (1 + N)
				 * depending on challenge_length */
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL1_CMD_GET_CHALLENGE */
	uint8_t con_id;		/*!< \brief Connection identifier of TasDispatcher */
	uint8_t err;		/*!< \brief Error code: TAS_PL_ERR_NO_ERROR, TAS_PL1_ERR_CMD_FAILED
				 * or TAS_PL1_ERR_DEV_ACCESS */
...
/*! \details Response: \ref tas_pl1rsp_set_device_key_st. Request description is followed by N data
 * words of a key. The key will be used if needed. */
/*! \ingroup Packet_Level_1 */
typedef struct {
	uint8_t wl;			/*!< \brief Request size (excluding header): 1 ... (1 + N)
					 * depending on key_length */
...
typedef struct {
	...
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL1_CMD_SET_DEVICE_KEY */
	uint8_t con_id;		/*!< \brief Connection identifier of TasDispatcher */
	uint8_t err;		/*!< \brief Error code: TAS_PL_ERR_NO_ERROR (key will be used by
				 * following device_connect) */
...
typedef struct {
	...
	uint16_t option;	/*!< \brief Connection option \ref tas_dev_con_feat_et but only
				 * _RESET, _RESET_AND_HALT, _UNLOCK */
	uint16_t reserved1;	/*!< \brief Reserved field: 0 */
} tas_pl1rq_device_connect_st;
...
typedef struct {
	...
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL1_CMD_DEVICE_CONNECT */
	uint8_t con_id;		/*!< \brief Connection identifier of TasDispatcher */
	uint8_t err;		/*!< \brief Error code: TAS_PL_ERR_NO_ERROR, TAS_PL1_ERR_DEV_ACCESS,
				 * TAS_PL1_ERR_DEV_LOCKED or TAS_PL1_ERR_CMD_FAILED (associated
				 * feat_used bit not set) */

	uint16_t feat_used;	/*!< \brief Connection option used \ref tas_dev_con_feat_et used
				 * (_RESET, _RESET_AND_HALT, _UNLOCK) */
	uint16_t reserved;	/*!< \brief Reserved field: 0 */
...
typedef struct {
	...
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL1_CMD_DEVICE_RESET_COUNT */
	uint8_t con_id;		/*!< \brief Connection identifier of TasDispatcher */
	uint8_t err;		/*!< \brief Error code: TAS_PL_ERR_NO_ERROR, or
				 * TAS_PL1_ERR_CMD_FAILED */
...
typedef enum {
	...
	TAS_CHT_BIDI = 0x03,	/*!< \brief Only use if the client always knows when to send and
				 * when to receive */
} tas_cht_et;
...
typedef struct {
	...
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL1_CMD_CHL_UNSUBSCRIBE */
	uint8_t chl;		/*!< \brief Channel number: 0 <= chl < MCD_CHL_NUM_MAX */
	uint8_t err;		/*!< \brief Error code: TAS_PL_ERR_NO_ERROR (unsubscribing an
				 * unsubscribed channel is no error) */
...
/*! \details Or in case of unidirectional communication also just a device to a client channel
 * message */
/*! \ingroup Packet_Level_1 */
typedef struct {
	uint8_t wl;		/*!< \brief Response size (excluding header): 1 */
...
typedef struct {
	...
	uint8_t stream;		/*!< \brief Trace stream identifier */
	uint8_t err;		/*!< \brief Error code: TAS_PL_ERR_NO_ERROR (unsubscribing an
				 * unsubscribed stream is no error) */
...
typedef enum {
	...
	TAS_TRCS_SYNC     = 1,	/*!< \brief Trace stream sync point for trace decoding (e.g. MTSC
				 * container) */
	TAS_TRCS_START_AC = 2,	/*!< \brief Trace stream start after trace hardware
				 * (re)configuration */
	TAS_TRCS_START_AR = 3,	/*!< \brief Trace stream start after an interruption by a reset */
	TAS_TRCS_START_AI = 4,	/*!< \brief Trace stream start after an interruption by a trace
...
typedef struct {
	...
	uint16_t pl1_cnt;		/*!< \brief Packet level 1 count returned by \ref
					 * tas_pl1rsp_pl0_end_st */
	uint16_t pl0_addr_map_mask;	/*!< \brief Address maps used by this PL1 packet. Bit 15
					 * represents TAS_AM15. */
...
typedef struct {
	...
	uint8_t addr_map;	/*!< \brief Address map: 0 is default for device access. Refer also
				 * to TAS_AM15 \ref tas_am15_am14.h */
	uint8_t reserved;	/*!< \brief Reserved field 0 */
...
typedef struct {
	uint8_t wl;		/*!< \brief Request size (excluding header): 1 */
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL0_CMD_WR8, TAS_PL0_CMD_WR16,
				 * TAS_PL0_CMD_WR32 */
	uint16_t a15to0;	/*!< \brief Address offset value which forms a full address with a
				 * base address from \ref tas_pl0rq_base_addr32_st or \ref
				 * tas_pl0rq_base_addr64_st */
...
typedef struct {
	uint8_t wl;		/*!< \brief Request size (excluding header): 0 */
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL0_CMD_RD8, TAS_PL0_CMD_RD16,
				 * TAS_PL0_CMD_RD32, TAS_PL0_CMD_RD64 */
	uint16_t a15to0;	/*!< \brief Address offset value which forms a full address with a
				 * base address from \ref tas_pl0rq_base_addr32_st or \ref
				 * tas_pl0rq_base_addr64_st */
} tas_pl0rq_rd_st;
...
typedef struct {
	...
	uint16_t a15to0;		/*!< \brief Address offset value which forms a full address
					 * with a base address from \ref tas_pl0rq_base_addr32_st or
					 * \ref tas_pl0rq_base_addr64_st */
...
/*! \brief Response to \ref tas_pl0rq_wr_st, \ref tas_pl0rq_wr64_st, \ref tas_pl0rq_wrblk_st, and
 * \ref tas_pl0rq_fill_st. */
/*! \ingroup Packet_Level_0 */
typedef struct {
	uint8_t wl;		/*!< \brief Request size (excluding header): 0 */
	uint8_t cmd;		/*!< \brief Command identifier: TAS_PL0_CMD_WRxx */
	uint8_t wlwr;		/*!< \brief 1 for wr, 2 for wr64, N for wrblk (0 means 256 if no
				 * error) data words written. */
	uint8_t err;		/*!< \brief Error code: \ref tas_pl_err_et */
} tas_pl0rsp_wr_st;

/*! \brief Response to \ref tas_pl0rq_rd_st, and \ref tas_pl0rq_rdblk_st. */
/*! \details Respons definition is followed by 0 to 255 data words. \ref tas_pl0rsp_rdblk1kb_st is
 * used for 256 data words. */
/*! \ingroup Packet_Level_0 */
typedef struct {
