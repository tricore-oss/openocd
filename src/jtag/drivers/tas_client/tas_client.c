...
static struct tas_client tas_client = {.max_pl2rq_pkt_size = 1024,
                                       .max_pl2rsp_pkt_size = 1024,
                                       .pl0_max_num_rw = 128};
...
static size_t mem_req_num = 0;
static tas_target_info_st target;
static bool harr = false;
static bool porst = false;
...
static int tas_client_reset(int trst, int srst) {
  ...
  if (srst && !porst) {
    ...
    if (porst) {
      if (harr) {
        err = tas_client_device_connect(&tas_client,
...
static int tas_client_op_connect(struct ocmts *ocmts) {
  ...
  if (jtag_reset_config & RESET_CNCT_UNDER_SRST) {
    err =
        tas_client_device_connect(&tas_client, TAS_DEV_CON_FEAT_RESET_AND_HALT);
  } else {
...
static int tas_client_read_byte(struct ocmts *ocds, uint8_t *data) {
  if (mem_req_num >= 256) {
    int ret =
        tas_client_execute_mem_reqs(&tas_client, 0, mem_reqs, mem_req_num);
    if (ret) {
      return ERROR_FAIL;
    }
    mem_req_num = 0;
