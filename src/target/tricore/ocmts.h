#ifndef OPENOCD_TARGET_AURIX_AURIX_OCMTS_H
#define OPENOCD_TARGET_AURIX_AURIX_OCMTS_H
...
struct ocmts_ops {
   ...
   * @param ocmts Pointer to the OCMTS structure
   * @param data 32-bit data to write/send
   * @return ERROR_OK on success, error code otherwise
   * @note RW Mode: Write word. COM Mode: Send word
   */
  int (*queue_io_write_word)(struct ocmts *ocmts, uint32_t data);
  /**
   ...
   * @note RW Mode only
   */
  int (*queue_io_write_hword)(struct ocmts *ocmts, uint16_t data);
  /**
  ...
  int (*queue_io_write_byte)(struct ocmts *ocmts, uint8_t data);
  /**
...
static inline int ocmts_queue_io_write_byte(struct ocmts *ocmts, uint8_t data) {
