...
struct ocmts_ops {
  ...
  int (*queue_io_write_block)(struct ocmts *ocmts, const void *buffer,
                              size_t count);
  /**
...
static inline int ocmts_io_write_block(struct ocmts *ocmts, uint32_t addr,
...
static inline int ocmts_io_read_block(struct ocmts *ocmts, uint32_t addr,
...
static inline int ocmts_io_read_u8(struct ocmts *ocmts, uint32_t addr,
                                   uint8_t *data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_io_write_u8(struct ocmts *ocmts, uint32_t addr,
                                    uint8_t data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_io_read_u16(struct ocmts *ocmts, uint32_t addr,
                                    uint16_t *data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_io_write_u16(struct ocmts *ocmts, uint32_t addr,
                                     uint16_t data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_io_read_u32(struct ocmts *ocmts, uint32_t addr,
                                    uint32_t *data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_io_write_u32(struct ocmts *ocmts, uint32_t addr,
                                     uint32_t data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_queue_write_block(struct ocmts *ocmts, uint32_t addr,
                                          const void *buffer, uint32_t size) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_queue_read_block(struct ocmts *ocmts, uint32_t addr,
                                         void *buffer, uint32_t count) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_queue_read_u8(struct ocmts *ocmts, uint32_t addr,
                                      uint8_t *data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_queue_write_u8(struct ocmts *ocmts, uint32_t addr,
                                       const uint8_t *data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_queue_read_u16(struct ocmts *ocmts, uint32_t addr,
                                       uint16_t *data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_queue_write_u16(struct ocmts *ocmts, uint32_t addr,
                                        const uint16_t *data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_queue_read_u32(struct ocmts *ocmts, uint32_t addr,
                                       uint32_t *data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_queue_write_u32(struct ocmts *ocmts, uint32_t addr,
                                        const uint32_t *data) {
  int retval = ocmts->ops->queue_io_set_address(ocmts, addr);
  if (retval != ERROR_OK)
    return retval;
...
static inline int ocmts_queue_io_write_block(struct ocmts *ocmts,
                                             const void *buffer,
                                             uint32_t count) {
  return ocmts->ops->queue_io_write_block(ocmts, buffer, count);
}

static inline int ocmts_queue_io_read_block(struct ocmts *ocmts, void *buffer,
                                            uint32_t count) {
  return ocmts->ops->queue_io_read_block(ocmts, buffer, count);
}

static inline int ocmts_queue_io_write_word(struct ocmts *ocmts,
                                            const void *data) {
  return ocmts->ops->queue_io_write_word(ocmts, data);
}

static inline int ocmts_queue_io_read_word(struct ocmts *ocmts,
                                           uint32_t *data) {
  return ocmts->ops->queue_io_read_word(ocmts, data);
}

static inline int ocmts_queue_io_write_hword(struct ocmts *ocmts,
                                             const void *data) {
  return ocmts->ops->queue_io_write_hword(ocmts, data);
}

static inline int ocmts_queue_io_read_hword(struct ocmts *ocmts,
                                            uint16_t *data) {
  return ocmts->ops->queue_io_read_hword(ocmts, data);
}

static inline int ocmts_queue_io_write_byte(struct ocmts *ocmts,
                                            const void *data) {
  return ocmts->ops->queue_io_write_byte(ocmts, data);
}
...
static inline int ocmts_queue_io_set_address(struct ocmts *ocmts,
                                             uint32_t addr) {
