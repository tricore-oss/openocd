...
int tricore_set_breakpoint(struct target *target, struct breakpoint *breakpoint,
                           bool restore);
int tricore_unset_breakpoint(struct target *target,
...
static int tricore_build_reg_cache(struct target *target,
...
static inline uint32_t tricore_get_reg_addr(struct target *target,
                                            uint16_t addr) {
  struct tricore_info *tricore = target_to_tricore(target);
  ...
  for (i = 0; i < target->reg_cache->num_regs; i++) {
    ...
    if (reg->dirty) {
      ret = ocmts_io_write_u32(tricore->ocmts,
                               tricore_get_reg_addr(target, arch_reg->addr),
                               arch_reg->value);
      if (ret) {
        LOG_TARGET_ERROR(target, "Failed to write core reg 0x%04x",
                         arch_reg->addr);
        return ret;
      }
      reg->dirty = false;
...
static inline void tricore_examin_debug_reason(struct target *target) {
  ...
  LOG_TARGET_DEBUG(target, "DBGSR halt: %d suspended: %d event: %d",
                   tricore->halted, tricore->suspended, tricore->active_event);
  ...
  case TRICORE_EVENT_TRIGGER7:
    if (tricore->events[tricore->active_event - TRICORE_EVENT_TRIGGER0].type ==
        TRICORE_EVENT_ADDRESS)
...
static int tricore_debug_entry(struct target *target) {
  ...
  ret |= ocmts_queue_read_u32(tricore->ocmts,
                              tricore_get_reg_addr(target, TRICORE_DBGSR),
                              &tricore->dbgsr);
  ret |= ocmts_queue_read_u32(
      tricore->ocmts, tricore_get_reg_addr(target, TRICORE_ICR), &tricore->icr);
  if (tricore->version == TRICORE_1_8 && tricore->has_virt) {
    ret |= ocmts_queue_read_u32(
        tricore->ocmts, tricore_get_reg_addr(target, TRICORE_VCON0), &vcon0);
    ret |= ocmts_queue_read_u32(
        tricore->ocmts, tricore_get_reg_addr(target, TRICORE_VCON1), &vcon1);
  }
...
static int tricore_debug_init(struct target *target) {
  ...
  /* Clear debug status */
  ret = ocmts_io_write_u32(tricore->ocmts,
                           tricore_get_reg_addr(target, TRICORE_DBGCFG),
                           virt_dbg_en | 0x8003);
  if (ret) {
...
static int tricore_poll(struct target *target) {
  ...
  if (tricore->version == TRICORE_1_8) {
    uint32_t bootcon;
    ret = ocmts_io_read_u32(tricore->ocmts,
                            tricore_get_reg_addr(target, TRICORE_BOOTCON),
                            &bootcon);
    if (ret) {
    ...
    uint32_t syscon;
    ret = ocmts_io_read_u32(
        tricore->ocmts, tricore_get_reg_addr(target, TRICORE_SYSCON), &syscon);
    if (ret) {
  ...
  if (tricore->dbgsr & 0x2 || bhalt) {
    LOG_TARGET_DEBUG(target, "Target is halted DGBSR: 0x%08x BHALT: %d",
                     tricore->dbgsr, bhalt);
...
int tricore_halt(struct target *target) {
  ...
  ret = ocmts_io_write_u32(tricore->ocmts,
                           tricore_get_reg_addr(target, TRICORE_DBGSR), 0x6);
  if (ret) {
...
int tricore_resume(struct target *target, bool current, target_addr_t address,
  ...
  ret = ocmts_io_write_u32(tricore->ocmts,
                           tricore_get_reg_addr(target, TRICORE_DBGSR), 0x4);
  if (ret) {
...
int tricore_step(struct target *target, bool current, target_addr_t address,
  ...
  struct breakpoint step_breakpoint = {
      .type = BKPT_HARD, .address = address, .length = 4};
...
err:
  LOG_TARGET_ERROR(
      target, "Failed to read memory at 0x%08" PRIx64 "[count=%u, size=%u]",
      address, count, size);
  return ret;
}
...
static int tricore_event_set(struct target *target, uint8_t event_id) {
  ...
  if (event->enable) {
    ...
    trxevt |= (event->bbm << 3) | (event->type << 12) | (event->compare << 13) |
              (event->access << 27);
    if (tricore->version == TRICORE_1_8) {
      trxevt |= 1;
    } else {
    ...
    if (event->compare == TRICORE_EVENT_RANGE) {
      ret = ocmts_queue_write_u32(
          tricore->ocmts,
          tricore_get_reg_addr(target, TRICORE_TRxADR(event_id + 1)),
          &event2->address);
    }
    ret |= ocmts_queue_write_u32(
        tricore->ocmts, tricore_get_reg_addr(target, TRICORE_TRxADR(event_id)),
        &event->address);
    ret |= ocmts_queue_write_u32(
        tricore->ocmts, tricore_get_reg_addr(target, TRICORE_TRxEVT(event_id)),
        &trxevt);
    ret |= ocmts_run(tricore->ocmts);
    if (ret) {
      return ret;
    ...
    ret = ocmts_io_write_block(
        tricore->ocmts, tricore_get_reg_addr(target, TRICORE_TRxEVT(event_id)),
        &(uint32_t[4]){0}, event->compare == TRICORE_EVENT_RANGE ? 4 : 2);
    if (ret) {
      return ret;
    }
  }
...
int tricore_set_breakpoint(struct target *target, struct breakpoint *breakpoint,
  ...
  tricore->events[event_id] = (struct tricore_event){
      ...
      .compare = (breakpoint->length > 4) ? TRICORE_EVENT_RANGE
                                          : TRICORE_EVENT_EQUALITY,
      .enable = true,
  };
  ...
  if (ret) {
    ...
    LOG_TARGET_ERROR(target, "Failed to set breakpoint at 0x%" PRIx64,
                     breakpoint->address);
    return ret;
  }

  breakpoint->is_set = true;
  LOG_TARGET_DEBUG(target, "Breakpoint set at 0x%" PRIx64 " using event %d",
                   breakpoint->address, event_id);
...
int tricore_unset_breakpoint(struct target *target,
  ...
  if (!tricore->events[event_id].enable) {
    LOG_TARGET_WARNING(target, "Breakpoint at 0x%" PRIx64 " already disabled",
                       breakpoint->address);
    return ERROR_OK;
  }
  ...
  ret = tricore_event_set(target, event_id);
  if (ret) {
    LOG_TARGET_ERROR(target, "Failed to unset breakpoint at 0x%" PRIx64,
                     breakpoint->address);
    return ret;
  }

  breakpoint->is_set = false;
  LOG_TARGET_DEBUG(target, "Breakpoint removed at 0x%" PRIx64,
                   breakpoint->address);
...
int tricore_add_watchpoint(struct target *target,
  ...
  event->bbm = false;
  event->compare =
      (watchpoint->length > 4) ? TRICORE_EVENT_RANGE : TRICORE_EVENT_EQUALITY;
  ...
  if (target->state == TARGET_HALTED) {
    ...
    if (ret) {
      ...
      if (watchpoint->length > 4)
        tricore->events[event_id + 1].enable = false;
      LOG_TARGET_ERROR(target, "Failed to set watchpoint at 0x%" PRIx64,
                       watchpoint->address);
      return ret;
    }
  }
  ...
  LOG_TARGET_DEBUG(target, "Watchpoint set at 0x%" PRIx64 " using event %d",
                   watchpoint->address, event_id);
...
int tricore_remove_watchpoint(struct target *target,
  ...
  if (!watchpoint->is_set) {
    LOG_TARGET_WARNING(target, "Watchpoint at 0x%" PRIx64 " not set",
                       watchpoint->address);
    return ERROR_OK;
  }
  ...
  if (target->state == TARGET_HALTED) {
    ...
    if (watchpoint->length > 4) {
      ret = ocmts_io_write_u32(
          tricore->ocmts,
          tricore_get_reg_addr(target, TRICORE_TRxEVT(event_id + 1)), 0);
      if (ret)
        LOG_TARGET_ERROR(target, "Failed to clear TRxEVT[%d]", event_id + 1);

      ret = ocmts_io_write_u32(
          tricore->ocmts,
          tricore_get_reg_addr(target, TRICORE_TRxADR(event_id + 1)), 0);
      if (ret)
  ...
  LOG_TARGET_DEBUG(target, "Watchpoint removed at 0x%" PRIx64,
                   watchpoint->address);
...
int tricore_hit_watchpoint(struct target *target,
  ...
  if (tricore->active_event >= TRICORE_EVENT_TRIGGER0 &&
      tricore->active_event <= TRICORE_EVENT_TRIGGER7) {
...
int tricore_examine(struct target *target) {
  ...
  if (!target_was_examined(target)) {
    uint32_t CPU_ID;
    ret = ocmts_io_read_u32(
        tricore->ocmts, tricore_get_reg_addr(target, TRICORE_CPU_ID), &CPU_ID);
    if (ret) {
      return ret;
    }
    ...
    if ((CPU_ID & 0xFF) == 0x31) {
      ...
      ret = ocmts_io_read_u32(
          tricore->ocmts, tricore_get_reg_addr(target, TRICORE_TTCON), &ttcon);
      if (ret) {
        return ret;
      }
      if (ttcon & 0x1) {
      ...
      LOG_TARGET_ERROR(target, "Unknown Tricore version found, CPU_ID: 0x%08x",
                       CPU_ID);
      return ERROR_TARGET_INVALID;
    }
